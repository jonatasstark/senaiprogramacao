...
int lerEntradaInteiro() {
    ...
    while (true) {
        std::getline(std::cin, entrada);
        if (!entrada.empty() && possuiApenasDigitos(entrada)) {
...
void organizarEquipamentosPorOrdemAlfabetica() {
    ...
    if (equipamentosDoPersonagem.size() >= 1) {
...
void removerEquipamento() {
    ...
    auto it = std::find_if(equipamentosDoPersonagem.begin(), equipamentosDoPersonagem.end(), [&nomeEquipamento](const Equipamento& e) {
        return e.nome == nomeEquipamento;
    });

    if (it != equipamentosDoPersonagem.end()) {
...
void exibirItensDoPersonagem() {
    ...
    if (pessoa[indexPersonagem].equipamentos.size() >= 1) {
...
void exibirHabilidadeDoPersonagem() {
    ...
    if (pessoa[indexPersonagem].habilidades.size() >= 1) {
...
// Função para buscar um equipamento na lista de equipamentos
